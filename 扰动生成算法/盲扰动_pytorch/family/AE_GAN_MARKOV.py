...
for epoch in range(1,epochs+1):
    ...
    for i,(batch_x,batch_y) in enumerate(MyDataLoader):
        ...
        correct=torch.sum((batch_adv_pred>=0.5)==batch_y).detach().numpy()
        epoch_correct+=correct
        #扰动量损失
        loss1=torch.sum(pertubation)/len(batch_x)

        #对抗样本损失
        loss2=nn.BCELoss()(batch_adv_pred,batch_adv_y)
...
torch.save(generatorModel,"generator/markov_generator_1_10000_epoch_10.pkl")
