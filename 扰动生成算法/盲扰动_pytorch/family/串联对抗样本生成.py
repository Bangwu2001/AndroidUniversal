...
#1.加载生成器模型
generatorModel=torch.load("generator/markov_generator_1_1_epoch_10_GANAnditer.pkl")
#2.加载本地替代模型分类器
...
evaluate_x_=evaluate_x.numpy()+universal_pertubation
evaluate_x_=torch.from_numpy(evaluate_x_).float()

pertubation=generatorModel(evaluate_x_)
evaluate_adv_x=evaluate_x_+pertubation*mask
evaluate_adv_x=evaluate_adv_x.int().float()

#对抗样本存储
with open("dataset/markov_adv_evaluate_1000_X_1_1_loop10_iterAndGAN","wb") as f:
    pickle.dump(evaluate_adv_x,f)

with open("dataset/markov_adv_evaluate_1000_Y_1_1_loop10_iterAndGAN","wb") as f:
