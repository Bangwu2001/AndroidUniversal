...
def train():
    ...
    torch.save(model,"generator/transform_v3_10_50_100")
    import matplotlib.pyplot as plt

    plt.title("train&test acc")
    plt.plot(train_acc,label="train")
    plt.plot(test_acc,label="test")
    plt.savefig("generator_result/transform_v3_10_50_100")
    plt.show()
...
def evaluate():
    ...
    benginData = torch.from_numpy(benginData).float()
    #加载训练好的生成器模型
    generatorModel = torch.load("generator/transform_v3_10_50_100")
    ...
    #对抗样本存储
    with open("generator_dataset/markov_evaluate_1000_X_adv_transform_v3_10_50_100","wb") as f:
        pickle.dump(evaluate_x_adv,f)
    with open("generator_dataset/markov_evaluate_1000_Y_adv_transform_v3_10_50_100","wb") as f:
