...
def train():
    ...
    for epoch in range(1,epochs+1):
        ...
        for index in range(batch_trainG_data.shape[0]//batch_size):
            ...
            x_add_universal=x+universal_per
            per=model(x_add_universal)
        ...
        #测试集
        with torch.no_grad():
            testG_data_add_universal = testG_data + universal_per

            per = model(testG_data_add_universal).int().float()
            per = torch.clamp(per + universal_per, min=0.) * mask
            per = torch.maximum(per, noise)
    ...
    torch.save(model,"generator/transform_v2_10_50_100")
    import matplotlib.pyplot as plt

    plt.title("train&test acc")
    plt.plot(train_acc,label="train")
    plt.plot(test_acc,label="test")
    plt.savefig("generator_result/transform_v2_10_50_100")
    plt.show()
...
def evaluate():
    ...
    noise[9, 10] = 1
    #加载训练好的生成器模型
    generatorModel = torch.load("generator/transform_v2_10_50_100")
    ...
    #生成扰动
    evaluate_x_add_universal = evaluate_x + universal_per
    per = generatorModel(evaluate_x_add_universal)
    per = torch.clamp(per + universal_per, min=0.) * mask
    per = torch.maximum(per, noise)
    per=per.int().float()
    print(per[0,0])
    ...
    #对抗样本存储
    with open("generator_dataset/markov_evaluate_1000_X_adv_transform_v2_10_50_100","wb") as f:
        pickle.dump(evaluate_x_adv,f)
    with open("generator_dataset/markov_evaluate_1000_Y_adv_transform_v2_10_50_100","wb") as f:
