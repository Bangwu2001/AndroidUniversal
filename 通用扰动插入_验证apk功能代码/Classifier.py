...
class MarkovClassifier():
    ...
    def __init__(self, clf_type):
        ...
        print("current classifier:",clf_type)
    ...
    def CNN1(self, x):
        ...
        model = torch.load("classifier/markov_classifier/Family_targetF_CNNModel.pkl")
    ...
    def CNN2(self, x):
        x = self.StandScale(x)
        x = torch.from_numpy(x).float()
        model = torch.load("classifier/markov_classifier/cnn.pkl")
        pred = model.forward(x)
        result = np.squeeze((pred >= 0.5).int().detach().numpy())
        return result
    ...
    def DNN(self, x):
        ...
        x = torch.from_numpy(x).float()
        model = torch.load("classifier/markov_classifier/dnn.pkl")
        pred = model.forward(x)
        result = np.squeeze((pred >= 0.5).int().detach().numpy())
        return result
    ...
    def RF(self, x):
        x = self.StandScale(x)
        x = x.reshape(x.shape[0], -1)
        model = joblib.load("classifier/markov_classifier/RF")
    ...
    def KNN1(self, x):
        x = self.StandScale(x)
        x = x.reshape(x.shape[0], -1)
        model = joblib.load("classifier/markov_classifier/KNN1")
        result = model.predict(x)
        return result
    ...
    def KNN3(self, x):
        x = self.StandScale(x)
        x = x.reshape(x.shape[0], -1)
        model = joblib.load("classifier/markov_classifier/KNN3")
        result = model.predict(x)
        return result
    ...
    def AdaBoost(self, x):
        x = self.StandScale(x)
        x = x.reshape(x.shape[0], -1)
        model = joblib.load("classifier/markov_classifier/adaboost")
        result = model.predict(x)
        return result

    def GCN(self, x):
        model = torch.load("classifier/markov_classifier/GCN_MAMA_FAMILY_20000.pt")
        model = model.cpu()
        result = []
...
if __name__ == "__main__":
    # 加载x
    data_path = "test_data/markov_evaluate_1000_X_sample_trainLoop2000_1_5000_selfDefined_v2"
    label_path = "test_data/markov_evaluate_1000_Y_sample_trainLoop2000_1_5000_selfDefined_v2"
